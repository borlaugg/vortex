    ...
    case 0: {
      ...
      case 2: {
        ...
        if (is_divergent) {
          ...
          warp[wid].ipdom_stack.emplace(warp[wid].tmask, ntaken_tmask, next_pc);
#endif
        }
      ...
      case 3: {
        ...
        if (stack_ptr != warp[wid].ipdom_stack.size()) {
          ...
          if (warp[wid].ipdom_stack.top().fallthrough) {
            next_tmask = warp[wid].ipdom_stack.top().orig_tmask;
            warp[wid].ipdom_stack.pop();
          } else {
            next_tmask = warp[wid].ipdom_stack.top().else_tmask;
            next_pc = warp[wid].ipdom_stack.top().PC;
            warp[wid].ipdom_stack.top().fallthrough = true;
          }
        }
#endif
      } break;
